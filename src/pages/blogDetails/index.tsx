...
import { motion, useScroll, useSpring } from "framer-motion"; // Import useScroll and useSpring
import { Share, Heart, MessageCircle, ArrowUp } from "lucide-react"; // Import ArrowUp
...
const BlogDetails = () => {
  ...
  // Handle navbar and scroll-to-top visibility on scroll
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Show scroll-to-top button when scrolled past a certain point (e.g., viewport height)
      setShowScrollTopButton(currentScrollY > window.innerHeight * 0.8);
    };

    window.addEventListener('scroll', handleScroll);
    document.documentElement.style.scrollBehavior = 'smooth'; // Keep smooth scrolling enabled
    ...
    return () => {
      ...
      // Optionally reset scroll behavior if needed elsewhere
  ...
  useEffect(() => {
    ...
    setTimeout(handleTocScroll, 100); // Run once shortly after render
    window.addEventListener("scroll", handleTocScroll); // Continue tracking
  ...
  if (isLoading) {
    return (
      <>
        {/* Scroll Progress Bar - Show even during loading */}
        <motion.div
          className="fixed top-0 left-0 right-0 h-1 bg-purple-600 origin-[0%]"
          style={{ scaleX, zIndex: 100 }} // Ensure it's above other elements
        />
        <motion.div
  ...
  if (error) {
    return (
      <>
        {/* Scroll Progress Bar - Show even during error */}
        <motion.div
          className="fixed top-0 left-0 right-0 h-1 bg-purple-600 origin-[0%]"
  ...
  return (
    <>
      {/* Scroll Progress Bar */}
      <motion.div
        className="fixed top-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500 to-blue-500 origin-[0%]"
        style={{ scaleX, zIndex: 100 }} // High z-index
      />

      {/* Animated Scroll to Top Button */}
      <motion.button
        className="fixed bottom-6 right-6 z-50 p-3 bg-purple-600 text-white rounded-full shadow-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-all"
        onClick={scrollToTop}
        initial={{ opacity: 0, y: 50, scale: 0.5 }}
        animate={{ opacity: showScrollTopButton ? 1 : 0, y: showScrollTopButton ? 0 : 50, scale: showScrollTopButton ? 1 : 0.5 }}
        exit={{ opacity: 0, y: 50, scale: 0.5 }}
        transition={{ type: "spring", stiffness: 500, damping: 30 }}
        aria-label="Scroll to top"
        whileHover={{ scale: showScrollTopButton ? 1.1 : 0.5 }}
        whileTap={{ scale: showScrollTopButton ? 0.95 : 0.5 }}
      >
        <ArrowUp size={24} />
      </motion.button>

      <motion.div
        className={`fixed top-0 left-0 right-0 z-40 md:bg-transparent md:backdrop-blur-md transition-transform duration-300 ${showNavbar ? 'translate-y-0' : '-translate-y-full'}`} // Lowered z-index slightly
        initial={{ y: '-100%' }}
        animate={{ y: showNavbar ? '0%' : '-100%' }}
        transition={{ duration: 0.3 }}
      >
